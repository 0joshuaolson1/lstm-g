...
Note that some languages may have more than one way to convert floats or doubles into strings. Some ways may not give the same value when converted back into a number, and the better ways may still only return strings close enough that the original value can be restored. In this library's code, the repr method is used instead of the str method when randomizing weights for automatic building (see Usage - Automatic Building) and in the toString method (see Usage - API), but the Decimal module would be needed to get the precise string version of a machine-stored floating-point number.
