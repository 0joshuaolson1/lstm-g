...
Porting this to other languages is highly encouraged. Monner and I would love to hear about uses anyone finds for the documentation and/or code, and feel free to ask questions! Note that some languages may have more than one way to convert floats or doubles into strings. Some ways may not give the same value when converted back into a number, and the better ways may still only return strings close enough that the original value can be restored. In this library's code, the repr method is used instead of the str method when randomizing weights for automatic building (see Usage - Automatic Building) and in the toString method (see Usage - API), but the Decimal module would be needed to get the precise string version of a machine-stored floating-point number.

An eventual goal of mine is to make a C/C++ library that generates C headers with hard-coded LSTM-g networks and optimized activation function approximations for maximum platform-independent efficiency. Of course, the computational complexity will be improved over the loop-heavy methods used here, and calculated values will be reused where possible. Maybe someone can beat me to it? Or try parallelizing it (although possible speedup is architecture-dependent), or extend LSTM-g in ways that LSTM has been modified, or even get a network working on an FPGA or neurocomputer...
...
All non-input units in the experiments in the paper were biased with a constant value of 1, and these bias connections counted toward the reported numbers of weights.
