THIS LIBRARY IS CURRENTLY INCORRECTLY WRITTEN. Check out https://github.com/cazala/synaptic instead for a better unofficial reference. Specifically, The XOR and Distracted Sequence Recall examples (which I'm not sure are right) show that it doesn't yet perform as expected. I won't do much LSTM-g work until I get over some depression problems, but those who want to try debugging this can find each other through the public Generalized LSTM contact page (https://docs.google.com/document/d/1NVQKHK-fkigwzXcusX8EZaw-LvP-TJAQdhrN6ts_ORo/edit). There you'll also find notes on stuff like ideas and LSTM-g-related papers. For anyone who wants to write their own XOR test, see XLBP's example/SequentialParity.java.
